...
if(isset($_POST['id'])){
    ...
    if(!preg_match("/^[0-9]*$/", $id) || empty($id)){
        $returnText->error = get_string('course_ipnan', $p);
    } else {
            ...
            if($array[0] != 'invalid'){
                ...
                foreach($array as $arr){
                    ...
                    if($arr[3]){
                        if($arr[4]){
                            $html .="
                                        <div class='otj-outer d-flex' onclick='window.location.href=`./teacher_hourslog.php?cid=$arr[1]&uid=$arr[2]&e=$type`'>
                                            <img src='./classes/img/HoursLog.png' class='tp-img'>
